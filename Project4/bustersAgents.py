...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        mostLikelyPos = [dist.argMax() for dist in livingGhostPositionDistributions]

        min_distance = float('inf')
        best_action = None
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            for ghostPos in mostLikelyPos:
                distance = self.distancer.getDistance(ghostPos, successor)
