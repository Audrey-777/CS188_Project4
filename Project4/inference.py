...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    observed_range = (X_RANGE - 1) + (Y_RANGE - 1) + MAX_NOISE
    variableDomainsDict = {GHOST0: positions, GHOST1: positions, PAC: positions, OBS0: list(range(observed_range + 1)), OBS1: list(range(observed_range + 1))}
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factorsList, joinedFactor = joinFactorsByVariable(factorsList, var)
            if len(joinedFactor.unconditionedVariables()) == 1:
                continue
            else:
                eliminatedFactor = eliminate(joinedFactor, var)
                factorsList.append(eliminatedFactor)

        if len(factorsList) == 0:
            from bayesNet import Factor
            res = Factor(queryVariables, [], bayesNet.variableDomainsDict())
            for assignment in res.getAllPossibleAssignmentDicts():
                res.setProbability(assignment, 1)
            return normalize(res)

        elif len(factorsList) == 1:
            resFactor = factorsList[0]
        else:
            resFactor = joinFactors(factorsList)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            truedist = manhattanDistance(ghostPosition, pacmanPosition)
            p = busters.getObservationProbability(noisyDistance, truedist)
            return p
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPos in self.allPositions:
            observedProb = self.getObservationProb(observation, pacmanPos, ghostPos, jailPos)
            self.beliefs[ghostPos] *= observedProb
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for newPos in self.allPositions:
            newBeliefs[newPos] = 0.0

        for oldPos in self.allPositions:
            oldPosBelief = self.beliefs[oldPos]

            if oldPosBelief > 0:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                for newPos in self.allPositions:
                    newBeliefs[newPos] += newPosDist[newPos] * oldPosBelief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        n = self.numParticles
        l = len(legalPositions)
        self.particles = [legalPositions[i % l] for i in range(n)]
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            w = self.getObservationProb(observation, pacmanPos, particle, jailPos)
            weights[particle] += w
    ...
    def elapseTime(self, gameState):
        ...
        for particle in particles:
            ...
            nextPos = newPos.sample()
            new_particles.append(nextPos)
