...
class BayesNet(object):
    ...
    def sameValuesDict(self, other): #比较两个网络在结构或取值域上的等价性
    ...
    def linearizeVariables(self): #对网络做拓扑排序（父节点总在子节点前）
        """
    ...
    def getCPT(self, variable): #获取或设置某个变量的条件概率表
        """
    ...
    def getReducedVariableDomains(self, evidenceDict): #基于观测证据缩减取值域
        """
    ...
    def getCPTWithEvidence(self, variable, evidenceDict=None): #在给定证据下提取单个变量的 CPT（行被删减）
        """
    ...
    def getAllCPTsWithEvidence(self, evidenceDict=None): #提取网络中所有变量在证据下的 CPT
        """
...
class Factor(object):
    ...
    def getProbability(self, assignmentDict): #给定一个（可能包含多余变量的）赋值字典，返回对应行的概率
        """ 
    ...
    def setProbability(self, assignmentDict, probability): #将指定赋值组合的概率设置为给定值（非负校验）
        """ 
    ...
    def getAllPossibleAssignmentDicts(self): #枚举当前因子所含变量的所有取值组合
        """
...
def normalize(factor): #对单一条件化赋值下的因子进行归一化，确保概率和为 1
    """
